...
<center>
<h1 style="color: white" >Self Drive Booking Details</h1>
<div class="table-responsive">
		<table>
			...
			$sql = "select * from selfDrive";
			$result = $conn->query($sql);
			
			if($result->num_rows>0){
				while($row=$result->fetch_assoc()){
					echo"<tr><td style='color: white ;text-align: center' class='bookingB'>".$row["BookingID"]."</td><td style='color: white ;text-align: center' class='bookingB'>".$row["VehID"]."</td><td style='color: white ;text-align: center' class='bookingB'>".$row["CusID"]."</td><td style='color: white ;text-align: center' class='bookingB'>".$row["PDate"]."</td><td style='color: white ;text-align: center' class='bookingB'>".$row["RDate"]."</td></tr>";
				}
			}
			else{
				echo"Empty Table";
